...
try:
    with open("cleanedFinal2.json", "r", encoding="utf-8") as f:
...
def publix_search_limited(search: str):
    data = PUBLIX_PRODUCTS
    search = " " + search.lower()
    products = [p for p in data if search in p["name"].lower()]

    sortedProducts = sorted(products, key=lambda p: (p["priceString"], p["name"].lower()))
    result = []
